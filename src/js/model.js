...
export const calcOutputAmonut = async function(inputAmount) {
    try {
        await fetch(`${API_URL}ticker?key=${KEY}&ids=${state.inputCurrency}&convert=${state.outputCurrency}`)
...
export const loadSearchResults = function(query,type) {
    let searchResults;
    if (type === 0 || type === 1 ) {
        searchResults =  state.currencies.filter(curr => curr.id.startsWith(query));
    }
    else if (type === 2 ) {
...
export const gethistoryPrice = async function () {    
    try {
    fetch(`${BASE_URL}/${API_VERSION}/candles/${state.selectedSymbol}/${state.timeRange}`)
    .then((response) => response.json())
